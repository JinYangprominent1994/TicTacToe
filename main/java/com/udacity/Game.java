...
public class Game {
    ...
    public String checkGameWinner(char[][] grid){
        ...
        } //check if all nine cells are occupied

        for (int p = 0; p < 3; p++) {
            if (grid[p][0] == 'x' && grid[p][1] == 'x' && grid[p][2] == 'x') {
                result = "X wins";
            }
        } //check "X" in columns

        for (int q = 0; q < 3; q++) {
            if (grid[0][q] == 'x' && grid[1][q] == 'x' && grid[2][q] == 'x') {
                result = "X wins";
            }
        } //check "X" in rows
        ...
        if (grid[0][2] == 'x' && grid[1][1] == 'x' && grid[2][0] == 'x') {
            result = "X wins";
        } //check "X" in diagonal lines



        for (int m = 0; m < 3; m++) {
            if (grid[m][0] == 'o' && grid[m][1] == 'o' && grid[m][2] == 'o') {
                result = "O wins";
            }
        }//check "o" in columns

        for (int n = 0; n < 3; n++) {
            if (grid[0][n] == 'o' && grid[1][n] == 'o' && grid[2][n] == 'o') {
                result = "O wins";
            }
        }//check "o" in rows
        ...
        if (grid[0][2] == 'o' && grid[1][1] == 'o' && grid[2][0] == 'o') {
            result = "O wins";
        }//check "o" in diagonal lines



        if(occupy.equals("full")&&result.equals("None")){
            result="Tie";
        }//if all cells are occupied, and both two players can win, and then the result is Tie
        return result;
    }
